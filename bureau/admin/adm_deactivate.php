...
if (! ($confirmed = ($_GET['submit'] == _("Confirm")) ) ) {
  echo '<h2>' . _('WARNING: experimental feature, use at your own risk') . '</h2>';
  ...
  <input type="submit" name="submit" value="<?php echo _("Confirm"); ?>" />
  </form><?php

  echo "<h3>" . _("Domains of user: ") . $r["login"] . "</h3>";
} else {
  if (!$_GET['redirect']) {
...
foreach ($domains as $key => $domain) {
  if (!$confirmed) echo '<h4>' . $domain . '</h4><ul>';
  $dom->lock();
  ...
  foreach ($r['sub'] as $k => $sub) {
	...
	$dom->lock();
	  ...
	  echo  "-- error in $sub.$domain: " . $err->errstr() . "\n";
	}
	$dom->unlock();
      }
    }
  }
  if (!$confirmed) echo '</ul>';
}

# 3. wrap up (?)
if ($confirmed) {
  echo "-- The following is a serie of SQL request you can run, as root, to revert the user's domains to their previous state.\n";
  echo $backup;
  echo "</pre>";
}
$cuid = $old_cuid;

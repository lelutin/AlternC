...
class m_dom {
  ...
  function del_domain($dom) {
    ...
    for($i=0;$i<count($classes);$i++) {
      if (method_exists($GLOBALS[$classes[$i]],"alternc_del_domain")) {
	$GLOBALS[$classes[$i]]->alternc_del_domain($dom);
      }
    }
    for($i=0;$i<count($classes);$i++) {
      if (method_exists($GLOBALS[$classes[$i]],"alternc_del_mx_domain")) {
	$GLOBALS[$classes[$i]]->alternc_del_mx_domain($dom);
      }
    }
    return true;
  }
  ...
  */
  function add_domain($domain,$dns,$noerase=0,$force=0) {
    ...
    // Lancement de add_dom sur les classes domain_sensitive :
     // Declenchons les autres classes.    
    for($i=0;$i<count($classes);$i++) {
      if (method_exists($GLOBALS[$classes[$i]],"alternc_add_domain")) {
	$GLOBALS[$classes[$i]]->alternc_add_domain($domain);
      }
    }
    for($i=0;$i<count($classes);$i++) {
  ...
  function whois($domain) {
	...
	case "be":
	  ...
	  if ($state==1 && substr($ligne,0,5)=="Name:") {
	    $tmp=trim(strtolower(ereg_replace("Name:","", $ligne)));
	    if ($tmp)
	      $server[]=$tmp;
	  }
	  break;
...
*/
  ...
  function edit_domain($dom,$dns,$mx) {
    ...
    if ($gesmx && !$r["mail"]) { // on a associé le MX : on cree donc l'entree dans LDAP
     ...
     for($i=0;$i<count($classes);$i++) {
      if (method_exists($GLOBALS[$classes[$i]],"alternc_add_mx_domain")) {
	$GLOBALS[$classes[$i]]->alternc_add_mx_domain($dom);
      }
	}
    }

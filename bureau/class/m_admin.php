...
class m_admin {
  ...
  function get_list($all=0) {
    // PATCHBEN pour ne voir que les comptes que l'on a créé (sauf root)
    global $err,$mem,$cuid;
  ...
  function checkcreator($uid) {
    ...
    // but ROOT (always uid 2000) is almighty
    if ($cuid==2000) {
      return true;
    }
   ...
   *
   */
  function add_mem($login, $pass, $nom, $prenom, $mail, $canpass=1, $type='default') {
	...
	if ($uid<=2000) $uid=2000;
      ...
      $db->query("INSERT INTO membres (uid,login,pass,mail,creator,canpass,type) VALUES ('$uid','$login','$pass','$mail','$cuid','$canpass', '$type');");
	...
	if (method_exists($GLOBALS[$classes[$i]],"alternc_add_member")) {
	  $GLOBALS[$classes[$i]]->alternc_add_member();
	}
      }
      $mem->unsu();
      return $uid;
   ...
   * 
   */
  function update_mem($uid, $mail, $nom, $prenom, $pass, $enabled, $canpass, $type, $type='default') {
    global $err,$db;
    global $cuid, $quota;
  ...
  function del_mem($uid) {
    ...
    $dom->alternc_del_member();
      ...
      for($i=0;$i<count($classes);$i++) {
	if (method_exists($GLOBALS[$classes[$i]],"alternc_del_member") && $classes[$i]!="dom") {
	  $GLOBALS[$classes[$i]]->alternc_del_member();
	}
      }

...
class m_mysql {
  ...
  */
  ...
  function add_db($dbn) {
    ...
    $db->query("GRANT ALL PRIVILEGES ON `".$dbname."`.* TO '".$lo."'@'%'");
  ...
  function del_db($dbn) {
    ...
    $db->next_record();
    $db->query("REVOKE ALL PRIVILEGES ON `".$dbname."`.* FROM '".$login."'@'%'");
    if ($db->f("cnt")==0) {
  ...
  function put_mysql_details($password) {
    ...
    $db->query("SET PASSWORD FOR '$login'@'%' = PASSWORD('$password')");
    return true;
  }
  ...
  function new_mysql($password) {
    ...
    $db->query("GRANT ALL PRIVILEGES ON `".$dbname."`.* TO '".$login."'@'%' IDENTIFIED BY '".$password."'");
    $db->query("CREATE DATABASE ".$dbname.";");
    return true;
  }
 ...
 function get_db_size($dbname) {
  ...
  function alternc_export($tmpdir) {
	...
	exec("/usr/bin/mysqldump --add-drop-table --quote-names -e -u".escapeshellarg($db->Record["login"])." -p".escapeshellarg($db->Record["pass"])." ".escapeshellarg($db->Record["db"])." |/bin/gzip >".escapeshellarg($filename));
	$str.="  <db>\n";
